...
// --- ADDED MOCK WALLETS COMPONENT ---
const MOCK_WALLETS = [
  { id: "1", user: "John Doe", role: "Vendor", balance: "$1,250.00", totalEarnings: "$15,400.00" },
  { id: "2", user: "Mike Smith", role: "Delivery", balance: "$340.50", totalEarnings: "$2,100.00" },
  { id: "3", user: "Sarah Jenkins", role: "Vendor", balance: "$8,900.00", totalEarnings: "$45,000.00" },
];

const WalletsTable = ({ onViewHistory }: { onViewHistory: (wallet: any) => void }) => {
  return (
    <div className="rounded-[11px] border border-[#D9D9D9] bg-white p-[27px_39px]">
      <div className="mb-6 flex items-center justify-between">
        <h3 className="font-sans text-xl font-bold text-black leading-5">User Wallets & Balances</h3>
      </div>
      <div className="w-full overflow-x-auto">
        <table className="w-full text-left font-sans text-[15px]">
          <thead className="bg-[#E9F3F3] text-black">
            <tr>
              <th className="p-4 font-semibold rounded-l-[10px]">User Name</th>
              <th className="p-4 font-semibold">Role</th>
              <th className="p-4 font-semibold">Current Balance</th>
              <th className="p-4 font-semibold">Total Earnings</th>
              <th className="p-4 font-semibold rounded-r-[10px]">Action</th>
            </tr>
          </thead>
          <tbody>
            {MOCK_WALLETS.map((wallet) => (
              <tr key={wallet.id} className="border-b border-[#E5E7EB] hover:bg-gray-50">
                <td className="p-4 font-medium text-black">{wallet.user}</td>
                <td className="p-4">
                  <span className={`rounded-full px-3 py-1 text-xs font-medium ${wallet.role === 'Vendor' ? 'bg-[#E9F3F3] text-[#06888C]' : 'bg-orange-50 text-orange-600'}`}>
                    {wallet.role}
                  </span>
                </td>
                <td className="p-4 font-bold text-[#01891C]">{wallet.balance}</td>
                <td className="p-4 text-[#6A717F]">{wallet.totalEarnings}</td>
                <td className="p-4">
                  <button 
                    onClick={() => onViewHistory(wallet)}
                    className="text-[#06888C] font-medium hover:underline"
                  >
                    View History
                  </button>
                </td>
...
export default function Transactions() {
  const [mainView, setMainView] = useState<"transactions" | "wallets">("transactions");
  
  // <-- NEW STATES FOR THE WITHDRAWAL MODAL -->
  const [isWithdrawalModalOpen, setIsWithdrawalModalOpen] = useState(false);
  const [selectedWallet, setSelectedWallet] = useState<any>(null);

  const handleOpenWalletHistory = (wallet: any) => {
  ...
  const transactionDisplays = useMemo((): TransactionDisplay[] => {
    return transactions.map((tx: any) => ({
      id: tx.id || "N/A",
      orderId: tx.orderId || tx.order?.id, 
      orderCode: tx.orderCode || tx.order?.orderCode,
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!orderCode && !orderId) return <span className="text-gray-400">N/A</span>;
          
          return (
            <Link 
              to={`/orders/${orderId}`} 
              className="font-sans text-[15px] font-medium text-[#06888C] hover:underline"
            >
              #{orderCode || "View Order"}
            </Link>
          );
        },
      },
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-8 border-b border-[#E5E7EB] px-2">
        <button
          onClick={() => setMainView("transactions")}
          className={`pb-4 font-sans text-lg font-bold transition-colors ${
            mainView === "transactions"
              ? "border-b-2 border-[#06888C] text-[#06888C]"
              : "text-[#898A8D] hover:text-[#06888C]"
          }`}
        >
          Transaction History
        </button>
        <button
          onClick={() => setMainView("wallets")}
          className={`pb-4 font-sans text-lg font-bold transition-colors ${
            mainView === "wallets"
              ? "border-b-2 border-[#06888C] text-[#06888C]"
              : "text-[#898A8D] hover:text-[#06888C]"
      ...
      {mainView === "transactions" ? (
        ...
        <WalletsTable onViewHistory={handleOpenWalletHistory} />
      )}
      ...
      {/* <-- ADDED THE WITHDRAWAL MODAL HERE --> */}
